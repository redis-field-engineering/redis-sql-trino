...
import com.redis.postgredis.ProxyServer.ServerStatus;
...
public final class PgAdapterTestEnv {
	...
	// Default fallback project Id will be used if one isn't set via the system
	// property.
...
//  }
	...
	public String getRedisURI() {
		if (redisURI == null) {
			redisURI = System.getProperty(TEST_HOST_PROPERTY, DEFAULT_REDIS_URI);
		}
		return redisURI;
	}
	...
	public String getDatabaseId() {
		...
		return String.format("%s_%d_%d", databaseId, System.currentTimeMillis(), new Random().nextInt(Short.MAX_VALUE));
	}
...
//  }
	...
	void initializeConnection(DataOutputStream out) throws Exception {
		// Send start message.
		{
			byte[] metadata = ByteBuffer.allocate(PROTOCOL_MESSAGE_SIZE).putInt(PROTOCOL_MESSAGE_SIZE)
					.putInt(INIT_PROTOCOL).array();
			out.write(metadata, 0, metadata.length);
		}

		// Send options.
		{
			String payload = "user\0" + System.getProperty("user.name") + "\0database\0" + getDatabaseId()
					+ "\0client_encoding\0UTF8\0DateStyle\0ISO\0TimeZone\0America/Los_Angeles\0extra_float_digits\0"
					+ "2\0\0";
			byte[] metadata = ByteBuffer.allocate(PROTOCOL_MESSAGE_SIZE)
					.putInt(payload.length() + PROTOCOL_MESSAGE_SIZE).putInt(OPTIONS_PROTOCOL).array();
	...
	void consumeStartUpMessages(DataInputStream in) throws java.io.IOException {
		// Get result from initialization request. Skip first byte since it is metadata
		// ('N' character).
