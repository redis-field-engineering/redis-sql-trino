...
public class RediSearchProject extends Project implements RediSearchRel {
  RediSearchProject(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, List<?
  ...
  public Project copy(RelTraitSet relTraitSet, RelNode input, List<RexNode> projects,
    ...
    return new RediSearchProject(getCluster(), traitSet, input, projects, relDataType);
  }
  ...
  public void implement(Implementor implementor) {
    ...
    final List<String> inFields = RediSearchRules.rediSearchFieldNames(getInput().getRowType());
    final RediSearchRules.RexToRediSearchTranslator translator =
        new RediSearchRules.RexToRediSearchTranslator((JavaTypeFactory) getCluster().getTypeFactory(), inFields);
    ...
    for (Pair<RexNode, String> pair : getNamedProjects()) {
      ...
      // "select *" present?
      hasSelectStar |= RediSearchConstants.isSelectAll(name);

      if (RediSearchRules.isItem(pair.left)) {
    ...
    List<String> newList = fields.stream()
        ...
        .filter(f -> !RediSearchConstants.ID.equals(f)).map(RediSearchRules::quote).collect(Collectors.toList());
