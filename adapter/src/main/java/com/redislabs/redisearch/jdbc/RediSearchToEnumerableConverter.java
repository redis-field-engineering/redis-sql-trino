...
/**
 * Relational expression representing a scan of a table in an RediSearch data source.
 */
public class RediSearchToEnumerableConverter extends ConverterImpl implements EnumerableRel {
  RediSearchToEnumerableConverter(RelOptCluster cluster, RelTraitSet traits, RelNode input) {
  ...
  public Result implement(EnumerableRelImplementor relImplementor, Prefer prefer) {
    ...
    final RediSearchRel.Implementor implementor = new RediSearchRel.Implementor();
    ...
    final Expression fields = block.append("fields",
        constantArrayList(Pair.zip(RediSearchRules.rediSearchFieldNames(rowType),
    ...
    final Expression table = block.append("table",
        implementor.table.getExpression(RediSearchTable.RediSearchQueryable.class));
    ...
    Expression enumerable = block.append("enumerable", Expressions.call(table,
        RediSearchMethod.REDISEARCH_QUERYABLE_FIND.method, ops, fields, sort, groupBy,
        aggregations, mappings, offset, fetch));
