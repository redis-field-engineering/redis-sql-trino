...
public class RediSearchAggregate extends Aggregate implements RediSearchRel {
  ...
  private static final Set<SqlKind> SUPPORTED_AGGREGATIONS = EnumSet.of(SqlKind.COUNT,
      SqlKind.MAX, SqlKind.MIN, SqlKind.AVG, SqlKind.SUM, SqlKind.ANY_VALUE);

  /**
   * Creates an ElasticsearchAggregate.
   */
  RediSearchAggregate(RelOptCluster cluster, RelTraitSet traitSet, RelNode input,
    ...
    for (AggregateCall aggCall : aggCalls) {
      ...
      if (!SUPPORTED_AGGREGATIONS.contains(kind)) {
        final String message = String.format(Locale.ROOT, "Aggregation %s not supported (use one " +
            "of %s)", kind, SUPPORTED_AGGREGATIONS);
        throw new InvalidRelException(message);
      }
    }
  ...
  public void implement(Implementor implementor) {
    ...
    for (AggregateCall aggCall : aggCalls) {
      ...
      String callString = toRediSearchAggregate(aggCall);

      implementor.addAggregation(aggCall.getName(), callString);
    }
  }

  /**
   * Most of the aggregations can be retrieved with single function. But currently only
   * one-to-one mapping is supported between sql agg and RediSearch
   * aggregation.
   */
  private static String toRediSearchAggregate(AggregateCall call) {
