...
import io.redisearch.querybuilder.RangeValue;
 ...
 * and convert it to {@link QueryNode}.
...
class PredicateAnalyzer {
   ...
   */
  static QueryNode analyze(RexNode expression) throws ExpressionNotAnalyzableException {
  ...
  private static class Visitor extends RexVisitorImpl<Expression> {
    ...
    public Expression visitCall(RexCall call) {
      ...
      case FUNCTION:
        ...
        // fall through
      default:
        String message = format(Locale.ROOT, "Unsupported syntax [%s] for call: [%s]", syntax,
            call);
        throw new PredicateAnalyzerException(message);
      }
    }
    ...
    private QueryExpression prefix(RexCall call) {
      Preconditions.checkArgument(call.getKind() == SqlKind.NOT, "Expected %s got %s",
          SqlKind.NOT, call.getKind());
    ...
    private QueryExpression postfix(RexCall call) {
      ...
      isColumn(a, call, RediSearchConstants.ID, true);
    ...
    private QueryExpression binary(RexCall call) {
      ...
      if (isColumn(pair.getKey(), call, RediSearchConstants.ID, false)) {
        switch (call.getKind()) {
        case EQUALS:
        case NOT_EQUALS:
          break;
    ...
    private static SwapResult swap(Expression left, Expression right) {
      ...
      if (literal == null || terminal == null) {
        String message = String.format(Locale.ROOT, "Unexpected combination of expressions " +
    ...
    private static boolean isColumn(Expression exp, RexNode node, String columnName,
  ...
  abstract static class QueryExpression implements Expression {
    ...
    public abstract QueryNode builder();
  ...
  static class CompoundQueryExpression extends QueryExpression {
    ...
    private final QueryNode builder;

    public static CompoundQueryExpression or(QueryExpression... expressions) {
      return new CompoundQueryExpression(false, QueryBuilder.union(queryNodes(expressions)));
    }

    private static QueryNode[] queryNodes(QueryExpression... expressions) {
      QueryNode[] nodes = new QueryNode[expressions.length];
      for (int index = 0; index < expressions.length; index++) {
        QueryExpression expression = expressions[index];
        nodes[index] = expression.builder();
      }
      return nodes;
    }
    ...
    public static CompoundQueryExpression and(boolean partial, QueryExpression... expressions) {
      return new CompoundQueryExpression(partial, QueryBuilder.intersect(queryNodes(expressions)));
    }

    private CompoundQueryExpression(boolean partial, QueryNode queryNode) {
      this.partial = partial;
      this.builder = Objects.requireNonNull(queryNode, "queryNode");
    }
    ...
    public QueryExpression not() {
      return new CompoundQueryExpression(partial, QueryBuilder.disjunct(builder()));
    }

    @Override
    public QueryExpression exists() {
      throw new PredicateAnalyzerException("SqlOperatorImpl ['exists'] " + "cannot be applied to "
    ...
    public QueryExpression notExists() {
      throw new PredicateAnalyzerException("SqlOperatorImpl ['notExists'] " + "cannot be applied "
          + "to a compound expression");
    }

    @Override
    public QueryExpression like(LiteralExpression literal) {
      throw new PredicateAnalyzerException("SqlOperatorImpl ['like'] " + "cannot be applied to a "
          + "compound expression");
    }
    ...
    public QueryExpression queryString(String query) {
      throw new PredicateAnalyzerException("QueryString " + "cannot be applied to a compound " +
          "expression");
    }
  ...
  static class SimpleQueryExpression extends QueryExpression {
    ...
    @Override
    public QueryExpression not() {
      builder = QueryBuilder.disjunct(builder());
      return this;
    }

    @Override
    public QueryExpression exists() {
      throw new UnsupportedOperationException("Exists expression not supported");
    }

    @Override
    public QueryExpression notExists() {
      builder = QueryBuilder.disjunct(exists().builder());
      return this;
    }

    @Override
    public QueryExpression like(LiteralExpression literal) {
      builder = QueryBuilder.intersect(getFieldReference(), literal.stringValue());
      return this;
    }
    ...
    @Override
    public QueryExpression notLike(LiteralExpression literal) {
      builder = QueryBuilder.disjunct(getFieldReference(), literal.stringValue());
      return this;
    }
    ...
    public QueryExpression equals(LiteralExpression literal) {
      ...
      if (value instanceof Number) {
        builder = QueryBuilder.intersect(getFieldReference(),
            Values.eq(((Number) value).doubleValue()));
      } else {
        builder = QueryBuilder.intersect(getFieldReference(), Values.tags(literal.stringValue()));
      }
      return this;
    }
    ...
    public QueryExpression notEquals(LiteralExpression literal) {
      Object value = literal.value();
      if (value instanceof Number) {
        builder = QueryBuilder.disjunct(getFieldReference(),
            Values.eq(((Number) value).doubleValue()));
      } else {
        builder = QueryBuilder.disjunct(getFieldReference(), Values.tags(literal.stringValue()));
      }
      return this;
    }

    private QueryNode range(LiteralExpression literal, Function<Double, RangeValue> function) {
      Object value = literal.value();
      if (value instanceof Number) {
        Number number = (Number) value;
        return QueryBuilder.intersect(getFieldReference(), function.apply(number.doubleValue()));
      }
      throw new UnsupportedOperationException("Range expression only supports numbers");
    }

    @Override
    public QueryExpression gt(LiteralExpression literal) {
      builder = range(literal, Values::gt);
      return this;
    }

    @Override
    public QueryExpression gte(LiteralExpression literal) {
      builder = range(literal, Values::ge);
      return this;
    }

    @Override
    public QueryExpression lt(LiteralExpression literal) {
      builder = range(literal, Values::lt);
      return this;
    }

    @Override
    public QueryExpression lte(LiteralExpression literal) {
      builder = range(literal, Values::le);
      return this;
    }
    ...
    @Override
    public QueryExpression isTrue() {
      throw new UnsupportedOperationException("isTrue expression not supported");
    }
  }
